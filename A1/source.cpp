...
//
// Created by Junpeng Gai on 16/07/2023
//
...
int main(int argc, char* argv[])
{
    ...
    cameraUp = vec3(0.0f, 1.0f, 1.0f);
        ...
        if (flag1 == true)
        {
            cameraLookAt = cameraLookAt - vec3(dx * dt * sensitivity, 0.0, 0.0);
        }
       
        if (flag2 == true)
        {
            cameraLookAt = cameraLookAt - vec3(0.0, dy * dt * sensitivity, 0.0);
        }


        if (flag3 == true)
        {
            fov += dx * dt * sensitivity*0.1;
            projectionMatrix = glm::perspective(fov,            // field of view in degrees
                800.0f / 600.0f,  // aspect ratio
